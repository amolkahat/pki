...
#cd /root/dogtag/
cd /root/amolkahat/
docker run --name pki1 --cap-add=NET_ADMIN -v /root/amolkahat/pki:/root/pki -dit pki /bin/sh -c "/sbin/ip addr add 172.17.0.07 dev eth0; bash"
docker run --name pki2 --cap-add=NET_ADMIN -v /root/amolkahat/pki:/root/pki -dit pki /bin/sh -c "/sbin/ip addr add 172.17.0.08 dev eth0; bash"
